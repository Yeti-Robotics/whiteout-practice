...
public class ShooterSubsystem extends SubsystemBase {
    ...
    public ShooterSubsystem() {
        shooterLeftTalon = new TalonSRX(Constants.SHOOTER_LEFT_MOTOR);
        shooterRightTalon = new TalonSRX(Constants.SHOOTER_RIGHT_MOTOR);
        shooterRightTalon.configSelectedFeedbackSensor(FeedbackDevice.QuadEncoder);
        shooterLeftTalon.setInverted(false);
        setPoint = (int)(Constants.MAX_RPM * .80); // 80%
    }
    ...
    public double getFlywheelRPM() {
        return (getRightEncoder() * Constants.PULLEY_RATIO * Constants.ENCODER_TIME_CONVERSION) /  Constants.ENCODER_RESOLUTION * Constants.QUAD_FACTOR;
    }
    ...
    public double getVelocityUnitsFromRPM(double RPM){
        return RPM /  Constants.PULLEY_RATIO *  Constants.ENCODER_TIME_CONVERSION / Constants.ENCODER_RESOLUTION;
    }
